...
if [ $result2 -gt $result1 ];then
	a=$((result2 - result1))
	myvar=`echo $a/$b  |bc -l`
	printf "packet discard rate increased by %0.2f per second\n" $myvar
        exit 1
elif [ $result2 -lt $result1 ];then
	a=$((result1 - result2))
	myvar=`echo $a/$b  |bc -l`
        printf "packet discard rate decreased by %0.2f per second\n" $myvar
else
	printf "packet discard rate is %0.2f per second \n"$myvar
fi
